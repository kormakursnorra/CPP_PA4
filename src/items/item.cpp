...
bool Item::applyItem(Creature* creature) {
    if (type == HEAL) {
        ...
        else if (newHp >= maxHp) creature->setHp(maxHp);
        else creature->setHp(newHp);
    } 
    else if (type == EMPOWER) {
        int attack = creature->getAttack();
        int maxAttack = creature->getAttack();
        ...
        else if (newAttack >= maxAttack) creature->setAttack(maxAttack);
        else creature->setAttack(newAttack);
    } 
    else if (type == BOLSTER) {
        int defence = creature->getDefense();
        int maxDefence = creature->getDefense();
        ...
        else if (newDefence >= maxDefence) creature->setDefence(maxDefence);
        else creature->setDefence(newDefence);
    } 
    else if (type == SPEED) {
        int speed = creature->getSpeed();
        int maxSpeed = creature->getSpeed();
        ...
        else if (newSpeed >= maxSpeed) creature->setSpeed(maxSpeed);
        else creature->setSpeed(newSpeed);
    } 
    return false;
}